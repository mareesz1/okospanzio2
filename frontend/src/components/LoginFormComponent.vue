<template>
  <img
    src="https://free4kwallpapers.com/uploads/originals/2015/06/16/hotel-in-santorini.jpg"
    class="image"
    alt=""
    id="image"
  />
          ...
          <div class="inputs">
            ...
            <div class="input-group" v-if="user.roles != 'guest'">
              <input
                id="code"
                name="code"
                :class="{ 'is-invalid': errors.code != null }"
                v-model="user.code"
                v-on:keyup.enter="authenticate()"
                type="password"
                class="form-control input"
            ...
            <p class="signup-link mt-2">
              Még nincs fiókod?<router-link to="/register"
                >Regisztrálj</router-link
              >
            </p>
          </div>
        </form>
